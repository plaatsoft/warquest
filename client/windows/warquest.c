...
// Webview code (pasted from old embeded browser example)
...
long EmbedBrowserObject(HWND hwnd)
{
	...
	if (!OleCreate(&CLSID_WebBrowser, &IID_IOleObject, OLERENDER_DRAW, 0, (IOleClientSite *)_iOleClientSiteEx, &MyIStorage, (void**)&browserObject))
	{
		...
		*((IOleObject **)ptr) = browserObject;
		...
		browserObject->lpVtbl->SetHostNames(browserObject, L"WarQuest", 0);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_CREATE) {
        ...
        int window_new_width = window_width * 2 - window_rect.right;
        int window_new_height = window_height * 2 - window_rect.bottom;
        SetWindowPos(hwnd, NULL, (GetSystemMetrics(SM_CXSCREEN) - window_new_width) / 2,
            (GetSystemMetrics(SM_CYSCREEN) - window_new_height) / 2, window_new_width,
            window_new_height, SWP_NOZORDER);
